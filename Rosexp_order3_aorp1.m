...
%
% avoiding the order reduction with p=1 to solve problem 
...
for ll=1:8
    ...
    for r=1:n
        ...
        % vecb(:,1) remaind the same as before. vecb(:,3) differs from the 
        % previous one in the boundary values
        % the boundary
    
        Un22=Un2.^2;
        UUn2=U.*Un2;
    
        vecb(:,2)=(-Uh2+Un22)/2-UUn2+Phh;
        vecb(1,2)=vecb(1,2)+Chu1;
        vecb(N-1,2)=vecb(N-1,2)+ChuN;
    
        vecb(:,3)=(-Uh2-Un22+2*UUn2)/k3+Vhn;
