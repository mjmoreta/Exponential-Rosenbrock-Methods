...
%
% with the standard method of lines to solve problem
...
for ll=1:8
    ...
    % They are Dirichlet. uu calculates the boundary values of u. The
    % first ones are calculated at the beginning of the loop
    Chu1=hdiv*uu(t,0);
    ChuN=hdiv*uu(t,1);
    ...
    for r=1:n
        ...
        % vecb contains the vectors that are multiplied by the  exponential 
        % funcions. The first column is zero, the second one is
        ...
        vecb(1,2)=vecb(1,2)+(Chu1+Chu11)/2;
        vecb(N-1,2)=vecb(N-1,2)+(ChuN+Chu1N)/2;
    
        vecb(:,3)=(UhPh-Un2Ph)/k+4*Vhn;
        vecb(1,3)=vecb(1,3)+(Chu1-Chu11)/k;
        vecb(N-1,3)=vecb(N-1,3)+(ChuN-Chu1N)/k;
        vecb(:,3)=vecb(:,3)/3;
